    ...
    <form method="GET" class="flex gap-3">
      ...
      <div class="flex gap-3 border-l">
        <div class="form-control ml-2 pl-2">
          <label class="label cursor-pointer space-x-1">
            <span class="label-text text-[15px]">show featured</span>
            <select
...
const currentList =
  route.params.type == "news" ? 1 : route.params.type == "stories" ? 2 : 0;
...
const update = (data) => {
  console.log(data);
  router.push({ name: "UpdateArticle", params: { id: data.id } });
};
